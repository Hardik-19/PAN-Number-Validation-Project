--- Comprehensive PAN Number Validation and Cleansing Using SQL Techniques  ---
...
as select distinct upper(trim(pan_number)) as pan_number
from stg_pan_numbers_dataset 
where pan_number is not null
and TRIM(pan_number) <> '';
...
case when vld.pan_number is null 
	then 'Invalid PAN' else 'Valid PAN' end as status
